...
{
    public class TenderType615Web : TenderWeb
    {
        public event Action<int> AddTender615;
        private bool Up;
        private string _purNum;

        public TenderType615Web(string url, JObject json, string purNum)
            : base(json, url)
        {
            _purNum = purNum;
        ...
        {
            var xml = GetXml();
            var firstOrDefault = T.Properties().FirstOrDefault(p => p.Name.Contains("pprf615"));
            if (firstOrDefault != null)
            {
                ...
                var purchaseNumber = ((string)tender.SelectToken("commonInfo.purchaseNumber") ?? _purNum ?? "").Trim();
                ...
                using (var connect = ConnectToDb.GetDbConnection())
                {
                    connect.Open();
                    var selectTender =
                        $"SELECT id_tender FROM {Program.Prefix}tender WHERE id_xml = @id_xml AND id_region = @id_region AND purchase_number = @purchase_number AND type_fz = 615";
                    ...
                    {
                        var selectDateT =
                            $"SELECT id_tender, doc_publish_date FROM {Program.Prefix}tender WHERE id_region = @id_region AND purchase_number = @purchase_number AND type_fz = 615";
