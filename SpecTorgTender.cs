...
namespace ParserTenders
{
    public class SpecTorgTender
    {
        ...
        public TypeSpecTorg TypeSpecTorgT;
        ...
        public void ParseAuction()
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                using (var connect = ConnectToDb.GetDbConnection())
                {
                    connect.Open();
                    var selectTend =
                        $"SELECT id_tender FROM {Program.Prefix}tender WHERE purchase_number = @purchase_number AND date_version = @date_version AND end_date = @end_date AND type_fz = 8";
                    ...
                    if (!string.IsNullOrEmpty(orgS))
                    {
                        ...
                        var orgFullName =
                            (navigator?.SelectSingleNode(
                             ...
                             "")
                            .Trim();
                        orgFullName = WebUtility.HtmlDecode(orgFullName);
                        ...
                        var orgTel =
                            (navigator?.SelectSingleNode(
                                ...
                                ?.Value ?? "").Trim();
                        orgTel = WebUtility.HtmlDecode(orgTel);
                        //WriteLine(orgTel);
                    ...
                    var idEtp = 0;
                    var etpName = @"Торговая Система ""Спецстройторг""";
                    var etpUrl = ParserSpecTorgWeb._site;
                    ...
                    cmd9.Parameters.AddWithValue("@type_fz", 8);
                    ...
                    {
                        urlAtt = $"{ParserSpecTorgWeb._site}{urlAtt}";
                        ...
                        if (!string.IsNullOrEmpty(strAtt))
                        {
                            ...
                            foreach (var att in attach)
                            {
                                ...
                                {
                                    urlF = $"{ParserSpecTorgWeb._site}{urlF}";
                                }
                    ...
                    else
                    {
                        var pName =
                            (navL?.SelectSingleNode(
                                ...
                                ?.Value ?? "").Trim();
                        pName = WebUtility.HtmlDecode(pName);
                        var okpd2Name = (navL?.SelectSingleNode(
                                ".//tr[starts-with(td[position()=1], \"Предмет\")]/td[last()]/text()")
                            ?.Value ?? "").Trim();
                        okpd2Name = WebUtility.HtmlDecode(okpd2Name);
                        var quantity =
                            (navL?.SelectSingleNode(
                                    ".//tr[td [position()=1]= \"Количество и единицы измерения:\"]/td[last()]")
                                ?.Value ?? "").Trim();
                        quantity = WebUtility.HtmlDecode(quantity);
                        var _lotPrice =
                            (navL?.SelectSingleNode(
                                     ".//tr[td [position()=1]= \"Цена за единицу:\"]/td[last()]")
                                 ?.Value ??
                             "").Trim();
                        _lotPrice = WebUtility.HtmlDecode(_lotPrice);
                        var lotPrice = UtilsFromParsing.ParsePrice(_lotPrice);
                        var insertLotitem =
                ...
                using (var connect = ConnectToDb.GetDbConnection())
                {
                    ...
                    var _UrlOrg =
                        (htmlDoc.DocumentNode
                            .SelectSingleNode("//td/a[@title = \"Просмотреть информационную карту участника\"]")
                            ?.Attributes["href"].Value ?? "").Trim();
                    var UrlOrg = $"{ParserSpecTorgWeb._site}{_UrlOrg}";
                    var orgS = DownloadString.DownL1251(UrlOrg);
                    if (!string.IsNullOrEmpty(orgS))
                    {
                        var htmlDocOrg = new HtmlDocument();
                    ...
                    cmd9.Parameters.AddWithValue("@id_etp", idEtp);
                    cmd9.Parameters.AddWithValue("@end_date", DateOpen);
                    cmd9.Parameters.AddWithValue("@scoring_date", DateOpen);
                    cmd9.Parameters.AddWithValue("@bidding_date", DateTime.MinValue);
                    cmd9.Parameters.AddWithValue("@cancel", cancelStatus);
                    cmd9.Parameters.AddWithValue("@date_version", DatePub);
                    cmd9.Parameters.AddWithValue("@num_version", 1);
        ...
        public void ParseRequestCustomer()
        {
            var str = DownloadString.DownL1251(UrlTender);
            if (!string.IsNullOrEmpty(str))
            {
                ...
                using (var connect = ConnectToDb.GetDbConnection())
                {
                    ...
                    switch (TypeSpecTorgT.Type)
                    {
                        ...
                        case SpecTorgType.RequestCustomer:
                            pwName = "Запрос предложений в форме ПДО";
                            break;
                        default:
                            pwName = "";
                            break;
        ...
        private void AddVerNumber(MySqlConnection connect, string purchaseNumber)
        {
            var verNum = 1;
            var selectTenders =
                $"SELECT id_tender FROM {Program.Prefix}tender WHERE purchase_number = @purchaseNumber AND type_fz = 8 ORDER BY id_tender ASC";
